          ...
          <DialogPanel class="mx-auto max-w-2xl transform divide-y divide-gray-800 overflow-hidden rounded-xl bg-secondary shadow-2xl transition-all">
            <Combobox @update:model-value="onSelect">
              ...
              <ComboboxOptions v-if="query === '' || filteredActions.length > 0" static class="max-h-80 scroll-py-2 divide-y divide-gray-800 overflow-y-auto">
                <li class="p-2">
                  <h2 v-if="query === ''" class="mt-4 mb-2 px-3 text-xs font-semibold text-muted">
                    Searches
                  </h2>
                ...
                <li v-if="query === ''" class="p-2">
                  ...
                  <ul class="text-sm text-muted">
                    <ComboboxOption v-for="action in consoleActions" :key="action.shortcut" v-slot="{ active }" :value="action" as="template">
              ...
              <div v-if="query !== '' && filteredActions.length === 0" class="p-2">
                ...
                <ul class="text-sm text-muted">
                  <li class="flex items-center rounded-md px-3 py-6">
                    <span v-if="!ai_response" class="ml-3 flex-auto truncate"> Press enter to get a response from AI assistant </span>
