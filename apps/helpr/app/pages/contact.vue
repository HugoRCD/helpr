        ...
        <div class="mx-auto max-w-lg lg:max-w-none">
          <form class="grid grid-cols-1 gap-y-6" @submit.prevent="submitForm">
            ...
            <div>
              ...
              <select id="ticket_type" v-model="ticket_type" required name="ticket_type" class="input w-full">
                <option value="" disabled selected>
                  {{ t("contact.ticket_type") }}
                </option>
                <option v-for="ticket in ticketValues" :key="ticket.value" :value="ticket.value">
                  {{ ticket.name }}
                </option>
              </select>
            </div>
